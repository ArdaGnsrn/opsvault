...
func ApplyRemote(ctx context.Context, cfg config.RetentionConfig, rcloneCfg config.RcloneConfig, databases []config.DatabaseConfig, log *slog.Logger) error {
	return ApplyRemoteWithPaths(ctx, cfg, rcloneCfg, databases, nil, log)
}

// ApplyRemoteWithPaths is like ApplyRemote but also prunes remote path archives.
func ApplyRemoteWithPaths(ctx context.Context, cfg config.RetentionConfig, rcloneCfg config.RcloneConfig, databases []config.DatabaseConfig, paths []config.PathConfig, log *slog.Logger) error {
	...
	for _, db := range databases {
		...
		if cfg.Remote.KeepDays > 0 {
			if err := pruneRemoteByAge(ctx, rcloneCfg, remotePath, db.Name, ".sql.gz", cfg.Remote.KeepDays, log); err != nil {
				log.Warn("failed to prune remote by age", "database", db.Name, "error", err)
			}
		}
		if cfg.Remote.KeepLast > 0 {
			if err := pruneRemoteByCount(ctx, rcloneCfg, remotePath, db.Name, ".sql.gz", cfg.Remote.KeepLast, log); err != nil {
	...
	for _, p := range paths {
		...
		remotePath := fmt.Sprintf("%s:%s", rcloneCfg.Remote,
			expandPath(rcloneCfg.Path, hostname, p.Name, ""))

		if cfg.Remote.KeepDays > 0 {
			if err := pruneRemoteByAge(ctx, rcloneCfg, remotePath, p.Name, ".tar.gz", cfg.Remote.KeepDays, log); err != nil {
				log.Warn("failed to prune remote by age", "path", p.Name, "error", err)
			}
		}
		if cfg.Remote.KeepLast > 0 {
			if err := pruneRemoteByCount(ctx, rcloneCfg, remotePath, p.Name, ".tar.gz", cfg.Remote.KeepLast, log); err != nil {
				log.Warn("failed to prune remote by count", "path", p.Name, "error", err)
			}
		}
	}
	return nil
...
func pruneRemoteByAge(ctx context.Context, rcloneCfg config.RcloneConfig, remotePath, name, ext string, keepDays int, log *slog.Logger) error {
	log.Info("pruning remote backups by age", "name", name, "keep_days", keepDays, "path", remotePath)

	args := []string{"delete", fmt.Sprintf("--min-age=%dd", keepDays), "--include", name + "_*" + ext}
...
func pruneRemoteByCount(ctx context.Context, rcloneCfg config.RcloneConfig, remotePath, name, ext string, keepLast int, log *slog.Logger) error {
	lsArgs := []string{"lsf", "--files-only", "--include", name + "_*" + ext}
	if rcloneCfg.RcloneConfig != "" {
	...
	log.Info("pruning remote backups by count", "name", name, "deleting", len(toDelete))
	...
	for _, f := range toDelete {
		...
		if out, err := exec.CommandContext(ctx, "rclone", delArgs...).CombinedOutput(); err != nil {
			...
			log.Info("deleted remote backup", "file", f, "name", name)
		}
	}
	return nil
}
